...
Objetivo:
- Validar diariamente la salud tecnica de la plataforma KDD.

Comprobaciones:
1) Contenedores core activos.
2) Topics Kafka necesarios.
3) Paths HDFS base.
4) Tablas Hive obligatorias.
5) Vistas Madrid y consultas smoke con degradacion segura.

Resultado esperado:
- Evidencia diaria en verde para operacion y entrega.
"""
...
with DAG(
    dag_id="kdd_daily_healthcheck",
    start_date=datetime(2026, 1, 1),
    schedule="@daily",
    catchup=False,
    default_args=default_args,
    on_failure_callback=notify_failure,
    tags=["big-data", "kdd", "healthcheck", "daily"],
) as dag:
    ...
    check_hdfs_paths = BashOperator(
        ...
        bash_command=(
            ...
            "\"hdfs dfs -test -d /data/raw && hdfs dfs -test -d /data/raw/nifi\""
        ),
    )
