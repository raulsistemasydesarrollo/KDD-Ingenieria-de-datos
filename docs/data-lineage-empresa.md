...
<img src="./lineage-diagrams/flow-general-fase-1.svg" alt="Flujo general fase 1" width="2100" />

<div style="page-break-after: always;"></div>

### Fase 2 - Procesamiento, IA y consumo en dashboard
<img src="./lineage-diagrams/flow-general-fase-2.svg" alt="Flujo general fase 2" width="2100" />

```mermaid
flowchart LR
  A1["01 Maestros CSV y grafo local - data/master y data/graph"] --> B2["02 GPS generator a nifi/input JSONL"]
  A1 --> S7["07 Seed raw en HDFS"]
  W3["03 Meteo raw Open-Meteo en nifi/raw-archive/weather"] --> K5
  B2 --> N4["04 NiFi archiva GPS raw"]
  N4 --> K5["05 Kafka raw y filtered"]
  K5 --> H6["06 Raw NiFi en HDFS"]
  S7 --> P8["08 Spark batch a Hive grafo y ML"]
  H6 --> P9["09 Spark streaming a Hive vistas Madrid"]
  P8 --> C10["10 Cassandra latest weather insights"]
  P9 --> C10
  P8 --> M11["11 Modelo IA en HDFS models"]
  C10 --> D12["12 Dashboard APIs"]
  A1 --> D12
```
...
<img src="./lineage-diagrams/points/p01.svg" alt="Grafico punto 01" width="1900" />

```mermaid
flowchart LR
  A["Entrada real CSV: MAD,Madrid Hub,ES,South Europe,high,40.4168,-3.7038"] --> B["Transformacion: normalizacion de maestros y catalogacion de campos"] --> C["Salida usada: warehouse_id=MAD, warehouse_name=Madrid Hub, criticality=high"]
```
...
<img src="./lineage-diagrams/points/p02.svg" alt="Grafico punto 02" width="1900" />

```mermaid
flowchart LR
  A["Entrada real GPS: event_id=gps_evt_1776117255, vehicle_id=TRUCK-009, warehouse_id=ZAR, delay_minutes=11"] --> B["Transformacion: gps-generator serializa JSON y escribe 1 linea por evento"] --> C["Salida real: nifi/input/gps_20260413T214832Z.jsonl con eventos JSONL"]
```
...
<img src="./lineage-diagrams/points/p03.svg" alt="Grafico punto 03" width="1900" />

```mermaid
flowchart LR
  A["Entrada real Open-Meteo: temperature_2m=18.7, precipitation=0.00, wind_speed_10m=5.6, weather_code=0"] --> B["Transformacion NiFi: InvokeHTTP + extraccion + normalizacion de atributos"] --> C["Salida real: raw-archive/weather JSON con temperature_c, precipitation_mm, wind_kmh"]
```
...
<img src="./lineage-diagrams/points/p04.svg" alt="Grafico punto 04" width="1900" />

```mermaid
flowchart LR
  A["Entrada real split GPS: vehicle_id=TRUCK-019, warehouse_id=MAD, delay_minutes=8, event_time=2026-04-04T14:41:14Z"] --> B["Transformacion NiFi: SplitText + EvaluateJsonPath + filename unico con UUID"] --> C["Salida real: raw-archive/gps/gps_..._uuid.jsonl sin sobrescritura"]
```
...
<img src="./lineage-diagrams/points/p05.svg" alt="Grafico punto 05" width="1900" />

```mermaid
flowchart LR
  A["Entrada real evento: vehicle_id=TRUCK-019, delay_minutes=8"] --> B["Regla NiFi: delay_minutes >= 5 para topic filtered"] --> C1["Salida real raw: topic transport.raw (todos los eventos)"]
  B --> C2["Salida real filtered: topic transport.filtered (solo retrasos >= 5)"]
```
...
<img src="./lineage-diagrams/points/p06.svg" alt="Grafico punto 06" width="1900" />

```mermaid
flowchart LR
  A["Entrada real raw-archive: gps_...jsonl y snapshots weather JSON"] --> B["Transformacion raw-hdfs-loader: copia a HDFS y prefija timestamp UTC"] --> C["Salida real HDFS: /data/raw/nifi/gps y /data/raw/nifi/weather"]
```
...
<img src="./lineage-diagrams/points/p07.svg" alt="Grafico punto 07" width="1900" />

```mermaid
flowchart LR
  A["Entrada real local: data/raw/gps_events.jsonl + data/master/* + data/graph/*"] --> B["Transformacion load-seed-data.sh: hdfs dfs -put"] --> C["Salida real HDFS seed: /data/raw/gps_events.jsonl, /data/master, /data/graph"]
```
...
<img src="./lineage-diagrams/points/p08.svg" alt="Grafico punto 08" width="1900" />

```mermaid
flowchart LR
  A["Entrada real batch: event_time=2026-04-04T14:41:14Z, delay_minutes=8, speed_kmh=56.2"] --> B["Transformacion Spark batch: cast timestamp, dedupe event_id, join maestros, GraphFrames, RF ML"] --> C["Salida real Hive: enriched_events, route_graph_metrics, route_shortest_paths, ml_delay_risk_scores"]
```
...
<img src="./lineage-diagrams/points/p09.svg" alt="Grafico punto 09" width="1900" />

```mermaid
flowchart LR
  A["Entrada real Kafka filtered: GPS y weather JSON"] --> B["Transformacion streaming: parse, watermark 20m, dedupe, ventana 15m y agregacion"] --> C["Salida real: Hive delay_metrics_streaming y weather_observations_streaming + Cassandra latest state"]
```
...
<img src="./lineage-diagrams/points/p10.svg" alt="Grafico punto 10" width="1900" />

```mermaid
flowchart LR
  A["Entrada real estado: vehicle_id=TRUCK-009, warehouse_id=ZAR, delay_minutes=11, speed_kmh=83.73"] --> B["Transformacion Cassandra: escritura por clave y snapshot operativo de baja latencia"] --> C["Salida real tablas: vehicle_latest_state, weather_observations_recent, network_insights_snapshots"]
```
...
<img src="./lineage-diagrams/points/p11.svg" alt="Grafico punto 11" width="1900" />

```mermaid
flowchart LR
  A["Entrada real entrenamiento: speed_kmh, vehicle_type, climate_temperature_c, congestion_avg_delay_minutes"] --> B["Transformacion ML: compara baseline_rf, tuned_baseline_rf, enhanced_rf y elige menor RMSE"] --> C["Salida real: /models/delay_risk_rf y tabla ml_delay_risk_scores con risk_level"]
```
...
<img src="./lineage-diagrams/points/p12.svg" alt="Grafico punto 12" width="1900" />

```mermaid
flowchart LR
  A["Entrada real APIs: /api/vehicles/latest, /api/weather/latest, /api/network/graph, /api/network/best-route"] --> B["Transformacion backend: filtros, joins, routing multiobjetivo, KPIs y enriquecimiento"] --> C["Salida visual real: mapa de flota, mapa de red, estadisticas, rutas candidatas y bloque IA"]
```
